...
class ATRStrategy(CommonStrategy):
    ...
    def next(self):
                    ...
                    if not self.position:
                        ...
                        if self.atr_stoploss.lines.trend_change[0] != self.atr_stoploss.lines.trend_change[-1]:
                            if trend == -1 and self.position.size > 0:
                                ...
                                # 平仓后立即卖出
                                # self.order = self.sell(size=self.baseLots)
                                self.trend_order = -2 if self.data.close[0] >self.data.open[0] else -1
        ...
        if self.trend_order > 0:
            if self.trend_order == 2 and self.data.close[0] < self.data.open[0]:
                self.order = self.buy(size=self.baseLots)
                self.trend_order = 0
            elif self.trend_order == 1 and self.data.close[0] > self.data.open[0]:
                self.order = self.buy(size=self.baseLots)
                self.trend_order = 0
        else:
            if self.trend_order == -2 and self.data.close[0] < self.data.open[0]:
