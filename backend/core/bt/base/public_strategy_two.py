...
from schemas.order_point import OrderPoint
...
class CommonStrategyTwo(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            print(f'{order_type} 订单执行完成，价格：{order.executed.price}, 交易价：{order.executed.value} '
                     f'当前关仓价：{self.datas[0].close[0]}, 数量：{order.executed.size}, 当前资金：{value}, 订单类型：{order.ordtype}')
    ...
    def notify_trade(self, trade):
        ...
        """
        try:
            print(f'交易完成，利润：{trade.pnl}, 数量：{trade.size}, 价格：{trade.price}')
            trader_dict = dict()
            # 精度
            digits = self.datas[0].digits[0]
            precision_format = f".{int(digits)}f"
    ...
    def stop(self):
        ...
        try:
            ...
            self.trader_report = {
                ...
                # 胜率
                "winRate": float(format(self.winning_trades / self.trade_count * 100,
                                        precision_format)) if self.winning_trades else 0,
                # 盈亏比
                "plr": float(format(profit_ratio_1, precision_format)),
                "avgProfit": float(
                    format(total_net_profit / self.trade_count, precision_format)) if total_net_profit else 0,  # 平均每次收益
