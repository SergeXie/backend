...
# 2. 构建LSTM模型
# ------------------------------------------------------------------------------
def build_lstm_model_status(max_seq_len: int, feat_dim: int = 7) -> Model:
    """构建适配的LSTM模型"""
    # 输入层：(最大序列长度, 特征维度)
    inputs = Input(shape=(max_seq_len, feat_dim))

    # LSTM层：捕捉时间序列趋势
    lstm1 = LSTM(units=64, return_sequences=True, dropout=0.2)(inputs)
    lstm2 = LSTM(units=32, return_sequences=False, dropout=0.2)(lstm1)  # 仅保留最后一个时间步的输出

    # 全连接层：融合特征并输出概率
    dense1 = Dense(64, activation="relu")(lstm2)
    dropout = Dropout(0.3)(dense1)  # 防止过拟合
    dense2 = Dense(32, activation="relu")(dropout)
    outputs = Dense(3, activation="sigmoid")(dense2)  # 3个概率值（0-1）

    # 编译模型
    model = Model(inputs=inputs, outputs=outputs)
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
        loss=tf.keras.losses.BinaryCrossentropy(),  # 多标签分类损失
        metrics=[tf.keras.metrics.BinaryAccuracy(name="acc")]
    )
    return model

def build_lstm_model_class(max_seq_len: int, feat_dim: int = 7) -> Model:
    """构建适配的LSTM模型"""
    # 输入层：(最大序列长度, 特征维度)
    inputs = Input(shape=(max_seq_len, feat_dim))
