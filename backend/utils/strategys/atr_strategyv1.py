...
class ATRStrategy(CommonStrategy):
    ...
    def __init__(self, indicator_params, goodsId=None, begin_time=None):
        ...
        self.atr_stoploss = ATRStopLoss(self.data,
                                        ...
                                        multiplier=self.indicator_params.get("Factor", 3.0))
        # 设置变量
        ...
        self.supremum = False  # 上界
        self.infimum = False  # 下界
    ...
    def next(self):
        ...
        # print(self.atr_stoploss.lines.target_dn[0], self.atr_stoploss.lines.target_up[0], trend, self.data.high[0], self.data.low[0])

        mimddle = self.data.high[0] + self.data.low[0]
        last_trend = self.atr_stoploss.lines.trend_change[-1]
        if last_trend == -1 and trend == 1:
            self.infimum = self.atr_stoploss.lines.target_up[0]
            self.supremum = mimddle - self.infimum
        elif last_trend == 1 and trend == -1:
            self.supremum = self.atr_stoploss.lines.target_dn[0]
            self.infimum = mimddle - self.supremum

        print(self.supremum, self.infimum)
