...
from utils.indicators.wm_predict_bymath import *
...
class WMTradingStrategy(CommonStrategy):
    ...
    def next(self):
        ...
        # ========== 平仓判断 ==========
        if self.position:
            if self.open_type == "short":  # M形态
                # 趋势反转 or 前浮动盈亏 >= 距离开仓价上涨超过 1 倍 ATR（达到预期利润区间）
                # self.four_price 第四个点价格
                # close > open_ 代表当前是阳线，市场可能要反弹了 → 先平掉。
                con1 = self.open_time_close - self.data.low[0] > self.four_to_three_price  # 一倍价差
                ...
                # print("short",current_dt, trend, stop)

                if con5 or con6:
                    # print(f"[平空] {current_dt}")
                    self.close()
                    self.con1 = False
                    self.open_type = None
                    self.open_time_close = None
        ...
        if not self.position and self.last_pattern_titles != pattern_titles:
            ...
            if pattern["end"] == current_dt.strftime('%Y-%m-%d %H:%M:%S'):
                ...
                m_weight = [weight[index] for index, i in enumerate(self.all_wm_pattern) if "M" in i['pattern_type']]
                m_p = probability(m_zigzag_points, datafrom='m', weight=m_weight)
                m_max_val = max(m_p['probabilities'])
                m_max_idx = m_p['probabilities'].index(m_max_val)
                print("m概率:", m_p, m_max_val, m_max_idx)

                # 在这里获取 start_two 时刻的趋势值
                trend_at_start_two = self.trend_map.get(pattern["start_two"])

                if "M" in pattern["value"] and trend_at_start_two == 1 and m_max_val>self.probability:
                    ...
                    self.stop_price1 = close - self.four_to_three_price * (m_max_val+1)  # 预测能到的最大值
