...
class ResponseWMpredictData(bt.Strategy):
    ...
    def stop(self):
        ...
        self.pattern_recognizer.get_dataset()
        return super().stop()
...
class WMPatternRecognizer:
    ...
    def analyze_zigzag_points(self, zigzag_points):
        ...
        if len(zigzag_points) >= 5:
            self._judgment_m_pattern(zigzag_points)
            self._judgment_w_pattern(zigzag_points)
    ...
    def _judgment_m_pattern(self, zigzag_points):
        ...
        if con1 and con3 and con4 and con5:
            self._add_dataset(self.dataset, zigzag_points[-5:])
    ...
    def _add_dataset(self, dataset, zigzag_points_1for5):
        """
